...
class UserController extends Controller
{
    ...
    public function do_login(Request $request)
    {
        ...
        //$userdata = DB::table('users')->where('email',$request->email)->where('type','beta')->first();
       $userdata = DB::table('users')->where('type','beta')->where('email',$request->email)->where('verify_email_code',$request->verify_email_code)->first();
        ...
        if($userdata){  
            ...
            else
            {
                ...
                $errors = ['Verification code incorrect'];
                return view('users.verify',compact('email','password'))->withErrors($errors);
            }
        }
        else
        {
    ...
    public function do_verify(Request $request)
    {
        ...
        $user = DB::table('users')->where('type','beta')->where('email',$request->email)->first();
        if($user && Hash::check($request->password, $user->password)) 
        {
            ...
            $subject = 'Nextlayer Mainframe Verification Code';
    ...
    public function update(Request $request)
    {
        ...
        $userdata = DB::table('users')->where('type','beta')->where('email',$request->email)->where('verify_email_code',$request->code)->first();
        if($userdata)
        {  
    ...
    {
        $merchant = DB::table('merchants_data')->where('id', $request->id)->where('merchant_backend_password', $request->old)->first();
        if($merchant)
        {
          	if($request->new===$request->confirm)
            {
              	$query=DB::table('merchants_data')->where('id', $request->id)->update([
                    'merchant_backend_password' => $request->new,
                ]);
              	DB::table('users')->where('id', $merchant->user_id)->update([
                    'password' => Hash::make($request->new)
                ]);
              	if($query)
                {
                  	return back()->with('success','Merchant Backend Password has been changed successfully.');
                }
              	else
                {
