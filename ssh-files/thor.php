        ...
        if(isset($_REQUEST['port']) && !empty($_REQUEST['port'])){
            if(isset($_REQUEST['username']) && !empty($_REQUEST['username'])){
                if(isset($_REQUEST['password']) || $_FILES['key']){
                    ...
                    echo json_encode(array("code" =>400, "message" => "Password or key is required Required"));
                    exit();
            ...
            echo json_encode(array("code" =>400, "message" => "Port Required"));
            exit();
...
if($type == 'start'){
    ...
    if(!empty($password)){
        ...
        if (!$ssh->login($username, $k)) {
            echo json_encode(["code" =>400, "message" => "Login Error key"]);
            exit();
        }
    }
