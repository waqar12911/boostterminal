...
if($type == 'start'){
    ...
    if(strpos($output, 'There is no screen to be resumed matching') !== false) {
        $ssh->write("screen -S Lightningd\n");
        $ssh->setTimeout(1);
        $ssh->write("sudo killall lightningd \n");
        $ssh->setTimeout(1);
        $ansi->loadString($ssh->read());
    ...
    else if(strpos($output, 'Server started with public key') !== false && strpos($output, 'Owning subdaemon channeld died') === false && strpos($output, 'Lost connection to the RPC socket.') === false) {
        $res = "Already Running";
    }else{
        if(empty($password) && $key)
        {
            $ssh->write("killall lightningd \n");
            $ssh->setTimeout(1);
        }else{
            $ssh->write("sudo killall lightningd \n");
            $ssh->setTimeout(1);
            ...
            if(strpos($sec, 'password for') !== false) {
                ...
                $ssh->setTimeout(1);
                $ansi->loadString($ssh->read());
                $rep = htmlspecialchars_decode(strip_tags($ansi->getScreen()));
                if(strpos($sec, 'invalid') !== false) {
                    echo json_encode(array("code" =>400, "message" => "Invalid Password"));
                }
            }
